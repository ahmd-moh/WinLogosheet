...
namespace Substation.Capture
{
    ...
    public sealed class RoiOcrEngine : IDisposable
    {
        ...
        /// <summary>
        /// Builds the engine now rather than at the first reading. Returns why it
        /// could not be built, or null when OCR is ready. Never throws: a node
        /// that cannot read still serves its stored hours and its QR code.
        /// </summary>
        public Exception Warm()
        {
            lock (_gate)
            {
                try
        ...
        public ReadingFrame Read(Bitmap screenshot, RoiConfig rois, string nodeId,
                                string sessionDate, int hour, string nodeVersion)
        {
